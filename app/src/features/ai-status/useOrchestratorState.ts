// useOrchestratorState — λ Wave-2 polling hook for the AI Status chip.
//
// Replaces ζ's mock useState. Polls the workspace's orchestrator runs
...
// animation already covers the visual fidelity case (δ's useSSE wires
...
function mapSubAgent(dto: SubAgentDTO): SubAgent {
  ...
  // fallback. Activity copy is intentionally minimal — α's SSE event
...
function emptyIdleState(): OrchestratorState {
  // Distinct from ζ's makeIdleState (which seeds a 4-hour-ago demo
