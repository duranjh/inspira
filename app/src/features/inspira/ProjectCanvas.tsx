...
function ProjectCanvasInner({
  ...
  // B1.2 / W3 δ — subscribe to α's SSE stream for this project. Re-emits
  // sub_agent.* and conflict.* events as window CustomEvents that the
  // chrome consumers (MultiAgentDots in TopicNode, ConflictBanner)
  // listen for. No-op when α isn't yet streaming (EventSource opens but
  // never receives data).
  useSSE(projectId);
  ...
  // "Inspira is working" signal. (Founder direction 2026-05-05:
  ...
  // On mobile the canvas-actions rail has too many pills; the screenshot
  // from the user's iPhone showed TE avatar + Tidy + Fit + Activity +
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={() => {
            setMoreOpen(false);
            // Founder direction 2026-05-06: Code is now its own
      ...
      {/* No empty-state overlay — founder direction 2026-05-05: even
      ...
      {/* Review chrome — relocated to the Artifact Viewer per the
          v4 redesign (2026-05-04). The canvas is now an editing
          surface only; approval lives where the code is. ConflictBanner
          only renders when α emits conflict.detected. */}
      <ConflictBanner />
      {activityOpen ? (
        <ActivityTimeline
          projectId={projectId}
...
// CanvasEmptyState removed — founder direction 2026-05-05: don't
...
function topicsToNodes(
  ...
  return topics.map((tp) => ({
    ...
    data: {
      ...
      // B1.2 / W3 δ — thread full Decision[] (not just statements) so the
      // card can render per-decision provenance dots. W2-θ attaches its
      // CommentTargetWrapper + CommentChip to each bullet, reading
