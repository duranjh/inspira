...
// Single-string contract the orchestrator SSE wiring targets: its onComplete
// callback dispatches `window.dispatchEvent(new CustomEvent(EVENT_NAME, { detail: payload }))`.
// The dev trigger button uses the same dispatch path so production wiring
// requires zero changes inside the drawer.
