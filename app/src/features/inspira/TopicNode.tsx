...
export type TopicNodeData = {
  ...
  icon: string;
  whyThisTopic?: string;
  // B1.2 / W3 δ — full Decision objects (not just statements) so each
  // bullet can render a per-decision provenance badge (gold dot for
  // AI-proposed, half-fill for AI-seeded-then-human-edited). W2-θ
  // attaches its CommentTargetWrapper + CommentChip to the same row,
      ...
      {data.decisions.length > 0 ? (
        <ul className="topic-node__decisions">
          {data.decisions.slice(0, 7).map((d) => (
            // δ + θ combined: per-decision ProvenanceBadge AND
