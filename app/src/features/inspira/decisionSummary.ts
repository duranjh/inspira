...
// `summary_json` match the orchestrator's `orchestrator_runs.summary_json`
// contract emitted on the SSE `decision_summary.ready` event. The Wave-2
// SSE wiring will assemble the outer DecisionSummary by joining
...
// sub-agent. Shape mirrors the orchestrator's emit at services/.../orchestrator.py:
...
// Cross-theme conflict resolved by the orchestrator. Mirrors its emit shape.
...
// The orchestrator_runs.summary_json shape (snake_case to match wire).
...
// What the drawer consumes. The SSE wiring will hand-assemble this from
// the orchestrator's stream + joins; the dev-only mock trigger emits this shape directly
// so the drawer requires zero changes when the SSE wiring lands.
export type DecisionSummary = {
  summary_json: DecisionSummaryJson;
  // Joined from v2_projects.title via theme.project_id, keyed by
  // theme_id. Missing labels fall back to the theme_id at render time.
  theme_labels: Record<string, string>;
  // From orchestrator_runs row (set by the SSE wiring).
  run_id: string;
  finishedAt: string;
  subAgentCount: number;
  // Computed from agent_runs + decision_provenance + feedback_items
  // joins — not part of the orchestrator's summary_json contract. Display-only.
  chips: DecisionChip[];
...
// the label join didn't carry one.
