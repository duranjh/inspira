...
export type TopicDetailProps = {
  ...
  // The parent canvas's project, used by the reasoning expander
...
export function TopicDetail({
  ...
  // Reasoning expander state. Lives at the TopicDetail level so
  ...
  >(() => new Map());
  // Reset reasoning state when the user navigates to a different topic in the
  ...
  // Listen to the useSSE window-event bridge for per-topic events.
  ...
  // REST fallback for cold-opens. ReasoningTrace fires this once
  ...
  const handleColorChange = useCallback(
    async (next: TopicColor | null) => {
      ...
      try {
        ...
        // kept the old one until the next page refresh.
    ...
    <CommentsProvider projectId={topic.project_id}>
      ...
      {realtime.contradictionEvent ? (
        ...
        <div className="td-body" ref={threadRef}>
          {state.kind === "initializing" ? (
            ...
            <div className="topic-detail__thread">
              {/* Additive layer above the existing Q&A thread.
        ...
        >
          ...
          {/* B3 (#077) — Private notes panel removed (product decision).
...
// Single decision row inside the topic-detail decisions column.
...
// SAVED_FLASH_MS constants were removed (product decision).
