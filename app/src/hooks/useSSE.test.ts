 ...
 *   - dispatches `inspira:sse:<name>` for each subscribed α event type
...
describe("useSSE", () => {
  ...
  it("dispatches inspira:sse:<name> for each subscribed α event", () => {
    act(() => {
      root.render(React.createElement(Probe, { projectId: "p_42" }));
    });
  ...
  it("dispatches decision.drafted with the full payload (η reasoning expander needs provenance)", () => {
    act(() => {
      root.render(React.createElement(Probe, { projectId: "p_42" }));
    });
    const es = MockEventSource.instances[0];
  ...
  it("ignores events outside the subscribed set on the default `message` channel", () => {
    ...
    // orchestrator.completed is owned by γ and not subscribed by this hook.
