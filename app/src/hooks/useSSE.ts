// B1.2 / W3 δ — SSE bridge for the canvas-review chrome.
//
...
//
// Uses Session α's verbatim event vocabulary — DO NOT invent names:
...
//
// δ only subscribes to the events it actually consumes (see
// SUBSCRIBED_EVENTS below). The others are owned by sibling sessions
// (γ for orchestrator.completed, ε for decision_summary.ready) —
// re-emitting them would create silent listener fan-out across
// modules that don't need them. η added decision.drafted because the
// Topic Detail reasoning expander needs the live append.
//
...
export function useSSE(projectId: string | null | undefined): void {
  useEffect(() => {
    ...
    // α may emit each event on its named channel rather than the default
