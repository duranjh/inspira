...
--
-- Use case: reset a dev account to a clean slate (no workspaces, no
-- projects, no clusters, no feedback, no orchestrator runs) so the
-- Onboarding Wizard fires correctly, then re-stamp the workspace
-- tier so Frontier-only Claude code-gen unlocks.
--
-- Run with:
--   psql "$DATABASE_URL" \
...
BEGIN
    ...
    -- from the temp table instead (substitution there is a
    -- known syntax-error trap).
    IF n_users = 0 THEN
