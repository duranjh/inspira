...
``/api/v2/orchestrator/*`` — α's original surface (6 routes):

- ``POST /prioritize``                            (admin+)
- ``GET  /prioritization-runs/{run_id}``          (member+)
- ``POST /run``                                   (admin+)
- ``GET  /runs``                                  (member+) — λ list endpoint
- ``GET  /runs/{run_id}``                         (member+)
- ``GET  /runs/{run_id}/events``                  (member+, SSE)

``/api/v2/projects/*`` — the gap-fill surface δ's frontend depends on
(closes #115 + #116, demo blocker for Sun 5/11):
...
- ``POST /projects/promote-from-cluster``         (admin+) — synthesizes a
  ...
  polls for the v2_projects row α writes early in the sub-agent, returns
...
- ``GET  /projects/{project_id}/events``          (member+, SSE) — proxies
  ...
  ``orchestrator_run_id`` from ``v2_projects.metadata_json``. Reuses α's
...
# raise this past ~50s without a proxy config change. α's _create_canvas
...
class TopicSeedBody(BaseModel):
    ...
    is a follow-up (~3-file ripple, demo doesn't depend on it).
    """
...
class StartCanvasBody(BaseModel):
    ...
    ``correction_note`` (founder direction 2026-05-05): when partners
    drag a card back into "In Progress" via the Kanban override
...
def make_orchestrator_router(
    ...
    # -----------------------------------------------------------
    # GET /runs  (member+) — λ workspace-scoped list for the AI
    ...
    async def promote_from_cluster_route(
        ...
        #    α's success-path write (orchestrator.py:469-484) hard-overwrites
    ...
    async def start_canvas_route(
        ...
        Founder direction (2026-05-04): auto-promoted Kanban shells
        ...
        # return the existing run_id idempotently — UNLESS the partner
        # explicitly asked for a rerun by providing a correction_note
        # (founder direction 2026-05-05). Rerun intent overrides
        ...
        # 1b. Concurrent sub-agent cap (founder direction 2026-05-04).
        ...
        # of the previous draft (founder direction 2026-05-05). The
...
def _find_promoted_v2_project(
    ...
    *,
    ...
    """Locate the v2_projects row α's ``_create_canvas`` writes for this run.

    α stamps ``metadata_json`` with ``orchestrator_run_id`` + ``theme_id``
