...
import → 10 batched calls → a few seconds. Acceptable for the v4
demo flow ("paste → see results in <10s").
...
# the GPT-5 family that holds up on classification tasks. Per the
# provider rule (memory: feedback_provider_rule_claude_codegen_only):
# non-code-gen LLM features always go to OpenAI regardless of tier.
