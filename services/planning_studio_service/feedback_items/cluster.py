...
0.78 so semantic paraphrases of the
...
faster with numpy). For a typical workload of ~50 clusters
...
def ensure_v2_projects_for_clusters(
    ...
    *,
    ...
    Product requirement: imports auto-create one canvas per theme —
    manual Promote-to-Project as the only path didn't match that. This
    helper creates one v2_projects row per cluster that doesn't already
    have one, with:
    ...
    Tier-capped (#172). To prevent the Kanban auto-spawn
    storm observed in production (179 cards from one 200-row CSV →
    ...
    explicitly. The Kanban populates regardless.
    """
    if not cluster_ids:
        return (0, 0)
    from ..store import now_timestamp
