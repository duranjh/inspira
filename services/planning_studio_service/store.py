...
class PlanningStudioStore:
    ...
    def _initialize(self) -> None:
        ...
        with self._connect() as connection:
            connection.executescript(
                ...
                -- See docs/product/architecture/data-model.md.)
                -- ============================================================
        ...
        # Seed only outside production — the "second brain commercialization"
        # demo project is handy in dev/tests but shouldn't ship to public
    ...
    def _initialize_v2_schema(self) -> None:
        ...
        truth: docs/product/architecture/data-model.md.
        """
        ...
        with self._connect() as connection:
            connection.executescript(
                ...
                -- W2-θ: append-only decision versions written by the comment-cascade
                ...
                -- W2-θ: per-cascade run record. Coordinates the BackgroundTask
    ...
    def _seed_defaults(self) -> None:
        ...
        with self._connect() as connection:
            connection.execute(
                ...
                """,
                (
                    "project-second-brain-commercialization",
                    "Second Brain and Planning Studio commercialization",
                    "Turn guided planning into durable sessions and reviewable planning artifacts.",
                    "discovery",
                    "project_manager",
                    json.dumps({"product_surface": "planning-studio", "sellable_asset_goal": True}),
                    created_at,
                    created_at,
                ),
            )
    ...
    def set_v2_project_artifact(
        ...
        Mirrors ``set_project_domain`` — JSON-level merge so γ's
        ``state``, α's ``orchestrator_run_id``, etc. survive each
        artifact write.
        """
        existing = self._get_v2_project(project_id)
        if existing is None:
    ...
    def _ensure_user_id_columns(self) -> None:
        ...
        if self._is_postgres:
            ...
            # ``user_id`` column, which is what triggered the 2026-04-23
            # demo-day health-degraded incident.
            with self._connect() as connection:
    ...
    # Schema source of truth: docs/product/architecture/data-model.md.
    # ================================================================
