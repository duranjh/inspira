...
Provider rule (``feedback_provider_rule_claude_codegen_only``): the
caller (route handler) does tier dispatch and passes the resolved
adapter. This helper is provider-agnostic — it never reads
``preferred_model_tier`` directly.
"""
