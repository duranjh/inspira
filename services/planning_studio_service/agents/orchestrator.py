...
def _max_concurrency() -> int:
    ...
    serialize the work and slow demos. Default 3 hits the sweet spot
    for top_n=5.
    """
...
def _create_canvas(
    ...
    *,
    ...
    - ``state`` — ``pending_review`` (success path) or ``generation_failed``.
      Session γ replaces this with a real enum column later; for now,
      reading via ``metadata.state`` is the contract.
    ...
    # The state-machine column landed in γ's W3 follow-up
...
async def _run_sub_agent_for_theme(
    ...
    *,
    ...
    # Founder direction 2026-05-05: when partners drag a card back into
...
def _moderate_conflict(
    *,
    ...
    Per the provider rule (memory:
    ``feedback_provider_rule_claude_codegen_only``), conflict moderation
    is non-code-gen so always uses OpenAI regardless of tier.
    """
