...
~24s for the same workspace size). Per the provider rule
(memory: ``feedback_provider_rule_claude_codegen_only``),
non-code-gen LLM features always use OpenAI regardless of tier.
...
The fallback ranking exists so partner demos still produce a
