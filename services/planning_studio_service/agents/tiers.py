...
# ---------------------------------------------------------------------------
# Monthly token caps per (plan, tier). Product decision (#080).
# ---------------------------------------------------------------------------
...
#
# Sizing rationale: caps assume ~3k output tokens per turn and bound the
# absolute monthly LLM spend per plan.
#
...
# Product decision: Free 1 / Pro 3 / Frontier 50 /
# Enterprise 100.
...
# the Kanban auto-spawn storm observed in production (179 cards from a
...
def tier_to_openai_model(tier: ModelTier) -> str:
    ...
    Per-tier mapping (per the LLM-tier overhaul):
    ...
    tier value scaling. The per-tier monthly-token caps (#080)
    backstop the absolute monthly cost.
...
def tier_to_reasoning_effort(tier: ModelTier) -> str | None:
    ...
    Per-tier policy (product decision):
...
# Code-generation pin for the artifact viewer endpoint. Per the provider rule
# (Claude only for code-gen on Frontier/Enterprise; non-code LLM features
# always use GPT regardless of tier), the artifact endpoint passes this as
...
def select_tier_after_cap_check(
    ...
    Fall-through chain (product decision):
