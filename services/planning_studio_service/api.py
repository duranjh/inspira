...
# is not defined`` the first time a user clicks "Map it" (seen on the
# 2026-04-23 demo smoke test).
...
# in fly.toml, none of the [document_bg] / [toolcall_retry] /
# [breakered_create] diagnostics from commit 395583f reached Fly.
...
class ExtractThemesBody(BaseModel):
    """YC v4 — pasted customer feedback items to cluster into themes.
...
class ProjectStateOverrideBody(BaseModel):
    ...
    recorded; the WHY is a nice-to-have. Founder direction 2026-05-05:
...
class ArtifactEditBody(BaseModel):
    """Body for the chat-driven artifact edit stream (W2 ι).
...
def create_app(store: PlanningStudioStore | None = None, adapter: Any = None) -> FastAPI:
    ...
    def _decision_author_user_id(decision: dict[str, Any]) -> str:
        ...
        ``proposed_by_user_id`` column — follow-up after demo.
        """
    ...
    #   network interface (10.0.0.219:4175, etc.) and still carry credentials
    ...
    # δ's canvas-review chrome depends on (closes #115 + #116):
    ...
    # W2-θ: comment cascade — text-select on a decision → comment →
    ...
    # W2 κ: Send-to-Linear / Send-to-GitHub export modals.
    ...
    def v2_kickoff(
        ...
        for idx, topic in enumerate(topics_raw):
            ...
            # B1 (YC v4 reframe) — persist any pre-populated Q&A turns
    ...
    def v2_get_project(
        ...
        Single-project getter introduced for the W2 κ export modals,
    ...
    def v2_list_topic_provenance(
        ...
        """Cited feedback items per decision for the η reasoning expander.
    ...
    def v2_topic_turn(
        ...
        # for the founder-locked policy (#075 iteration-4 fix for
        ...
        # 2026-04-26 (founder feedback): the auto-synthesis heuristic
    ...
    async def v2_kickoff_stream(
        ...
        async def _kickoff_generator():
            ...
            try:
                ...
                # demo audience assumes the app hung.
                ...
                # same phrase for 10 minutes (caught on the 2026-04-24
                # iPhone test). New script ramps from reassuring →
                ...
                # 10 ticks = ~30s total. R10 demo-window hardening
                # (engineering plan §3 R10): the W2/F5 ingestion
                # pipeline + worker has landed, so the original 90s
                # cap is back. The 30s floor was a tactical hedge;
                # the structural fix is now in place. Reverted from
                # 10 → 30 in W2/F5 (2026-05-02).
                    ...
                    try:
                        ...
                        yield format_sse(
                            "heartbeat",
                            {
                                ...
                                # R10: surface elapsed time so the FE
                                # can render a count-up alongside the
                                # rotating "thinking…" copy. Partners
    ...
    async def v2_topic_turn_stream(
        ...
        async def _turn_generator():
            ...
            try:
                ...
                # R10 demo-window hardening (engineering plan §3 R10):
                # 30 ticks × 3s = 90s hard cap. Reverted from the
                # tactical 10-tick (30s) hedge; W2/F5 ingestion +
                # worker is the structural fix and now in place.
                    ...
                    try:
                        ...
                        yield format_sse(
                            "heartbeat",
                            {
                                ...
                                # R10: see the kickoff stream comment
                                # for why elapsed_s lands on heartbeats.
                                "elapsed_s": step * 3,
                            },
                        )
                        step += 1
            ...
            try:
                ...
                # 2026-04-26 (founder feedback): proposed_decisions
    ...
    # Cap shares the existing business_plan_usage table per founder lock-in
    # 2026-04-29 — Pro 1 doc/mo (any type), Frontier 100/mo. Strict-block
    ...
    def _run_document_generation(
        ...
        Founder lock-in (2026-04-29): all 7 doc-type calls pin to
        ...
        try:
            ...
            # OpenAI-only by founder lock-in, NOT routed through
            ...
            # 5. Conditional cap increment. Per founder lock-in: only
    ...
    def v2_document_generate(
        ...
        4. Strict cap gate: 429 if business_plan_usage >= cap (Pro 1, Frontier
           100). Blocks regenerates too — founder lock-in 2026-04-29.
        ...
        # cap counter (business_plan_usage) per founder lock-in. The
        ...
        # Strict cap gate. Founder lock-in: 429 ANY POST when at cap,
    ...
    def v2_extract_themes(
        ...
        """YC v4 — cluster pasted customer feedback into 3-5 themes.
        ...
        Available to all tiers — the demo depends on it for Free users.
        """
    ...
    def v2_list_templates() -> dict[str, Any]:
        ...
        # demo template for the YC application.
    ...
    #
    # Gated on ``project_state == "approved"`` (γ's state machine is the
    ...
    def _artifact_assert_software_and_entitled(
        ...
        Founder reframe (2026-05-04): the **artifact (code) IS the
    ...
    def v2_get_artifact(
        ...
        as γ's transition endpoint. Don't leak existence.
        """
        _require_owned_project(project_id, user)
    ...
    # ``user_access_tokens`` table the other worktree is building.
