...
class GitHubInstallBody(BaseModel):
    """Re-confirm an existing installation. Used when the user
...
def make_connectors_router(
    ...
    async def github_repo_context_route(
        ...
        Product decision: before starting a canvas, Inspira pulls the
        latest repo state from GitHub so it never generates against a
        stale snapshot. Canvas spawn (`promote-from-cluster`) calls
    ...
    # modals (W2). Read/write the credential row's metadata_json.
    # -----------------------------------------------------------
