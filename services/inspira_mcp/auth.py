...
3. We look up the hash in ``user_access_tokens`` (a table owned by a
   concurrent agent in another worktree — see the task brief).
...
- A DB dump therefore cannot be replayed as live PATs. The only way to
  mint a token is the explicit issue flow (owned by the other agent).
...
The ``user_access_tokens`` table is being built in a parallel worktree.
Until that lands and merges, the lookup here gracefully catches the
"no such table" error (both backends) and returns a 401. This keeps the
main service bootable during the interleave and simplifies the merge —
no knock-on schema reset is required.
"""
...
def _hash_token(raw: str) -> str:
    ...
    Matches the scheme the token-issuing agent is expected to use:
...
def resolve_bearer_token(store: Any, authorization_header: str | None) -> str:
    ...
    - ``store`` is a ``PlanningStudioStore``. We use ``store._connect``
      ...
      store method and the concurrent-agent table does not yet have a
      dedicated store helper.
    ...
    try:
        ...
        # The user_access_tokens table hasn't landed yet (the PAT agent
        # is working in a separate worktree). Treat every request as
