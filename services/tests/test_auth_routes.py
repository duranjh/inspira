...
class SignupTests(unittest.TestCase):
    ...
    def test_signup_without_terms_accepted_returns_400(self) -> None:
        ...
        # No user was created.
        self.assertIsNone(self.store.get_user_by_email("no-terms@anthropic.ai"))
        ...
        response2 = self.client.post(
            "/api/auth/signup",
            json={
                "email": "missing-terms@anthropic.ai",
                "password": "password123",
            },
        )
    ...
    def test_signup_with_terms_accepted_persists_timestamp(self) -> None:
        ...
        response = self.client.post(
            "/api/auth/signup",
            json={
                "email": "accepted@anthropic.ai",
                "password": "password123",
                "terms_accepted": True,
            },
        )
        self.assertEqual(response.status_code, 201)
        stored = self.store.get_user_by_email("accepted@anthropic.ai")
...
class SignupReservedTldTests(unittest.TestCase):
    ...
    def test_real_tld_still_accepted(self) -> None:
        ...
        hasn't accidentally blocked everything. Uses ``anthropic.ai`` which
        is intentionally outside the reserved set (the existing auth tests
        use ``@example.com``, which now fails — see below)."""
        response = self.client.post(
            "/api/auth/signup",
            json={
                "email": "ok@anthropic.ai",
                "password": "password123",
                "terms_accepted": True,
            },
        )
...
class TransferAnonErrorCodeSplitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        signup_and_login(
            self.client, email="jane@anthropic.ai", password="password123",
        )
...
class LoginRateLimitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        signup_and_login(
            self.client, email="rl@anthropic.ai", password="password123",
        )
        self.client.cookies.clear()
    ...
    def test_eleventh_login_attempt_in_a_minute_is_rate_limited(self) -> None:
        ...
        for i in range(10):
            response = self.client.post(
                "/api/auth/login",
                json={"email": "rl@anthropic.ai", "password": f"wrong-{i}"},
            )
            self.assertIn(
        ...
        final = self.client.post(
            "/api/auth/login",
            json={"email": "rl@anthropic.ai", "password": "wrong-final"},
        )
        self.assertEqual(
