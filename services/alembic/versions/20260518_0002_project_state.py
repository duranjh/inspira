...
- ``roi_score``      — int populated by the orchestrator;
...
schema is forward-compatible — the orchestrator can flip a project
to it once the post-W4 summary feature lands without another
migration.
...
existing user-authored projects are already shipping/shipped, not
...
production INSERTs MUST set state explicitly:
  ...
  - The orchestrator-driven path (F7) writes
...
Drop indexes + columns. Dialect-aware drops mirror the earlier fix
...
def upgrade() -> None:
    ...
    if dialect == "postgresql":
        ...
        #   2. Backfill: pull state from metadata_json if the
