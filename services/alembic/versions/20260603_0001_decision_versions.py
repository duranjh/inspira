"""Add decision_versions table + decisions.current_version_int for the W2 comment cascade.
...
Wave 2 ships the highlight-and-comment + cascade flow. When a user
...
# Re-chained: was "20260518_0001"; bumped to chain after the
# connector_credentials_metadata migration so the alembic tree stays
# linear. Originally created in parallel to the project_state and
# connector_credentials_metadata migrations, producing a 3-way fork
# alembic upgrade refuses to resolve. The decision_versions table is
# orthogonal to both, so it runs cleanly on top of either ancestor.
