...
All variables the app reads are documented in `.env.example` with inline
...
| `INSPIRA_FRONTEND_URL` | Yes | Public URL of your frontend — used in share links and transactional emails |
...
- Sentry (if `SENTRY_DSN` set) — uncaught backend exceptions + performance.
...
- [`docs/deployment/security-audit.md`](docs/deployment/security-audit.md) — pre-deploy audit findings.
