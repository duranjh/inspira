...
Both paths are idempotent against each other. An earlier internal
design doc (not included in this repo) described an aspirational model
with workspaces, privacy flags, and other fields that are not yet in
the live schema — this document captures the **current state** only.
...
`project-demo-bootstrap` row at boot
(`store.py:369`).
...
**Known caveat:** a number of raw CRUD helpers in `store.py` still
accept `user_id` as a kwarg and intentionally ignore it (marked
`_ = user_id`) because the HTTP layer pre-checks ownership.
