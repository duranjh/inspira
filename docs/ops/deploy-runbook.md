...
1. **`fly deploy` does NOT run migrations.** Never merge a PR that
   ...
   (`services/scripts/check_migrations.py`) is not wired into any CI
   workflow — run it manually against the unpooled DB URL before any
   deploy. If you skip running it, you lose the safety net.
...
|------------------------------|--------------------|----------------------------------|
| Backend (`services/`)        | Fly.io (`inspira-backend`, region `iad`) | Manual `flyctl deploy --remote-only` (no deploy workflow ships in this repo) |
...
6. Deploy the backend manually:
   - Run `services/scripts/check_migrations.py` against
     `DATABASE_URL_PROD`. If the DB is behind the code, stop — run
     migrations first; do not deploy.
   - Run `flyctl deploy --remote-only` (from `services/`, or with
     `--config services/fly.toml --dockerfile services/Dockerfile`
     from the repo root).
   - The Cloudflare Pages deploy fires on its own via its native GitHub
     integration (no manual step needed).
...
| Secret                      | Purpose                                 |
|-----------------------------|-----------------------------------------|
| `DATABASE_URL_PROD`         | Read-capable unpooled Neon URL used by the pre-deploy drift check (`services/scripts/check_migrations.py`), run manually before deploying. Can be the same value as `DATABASE_URL` — storing it as a separate secret lets you swap it to a read-only role without touching runtime config. |

When absent, run the drift check against the runtime `DATABASE_URL`
instead — just don't deploy without running it.
...
Run the drift check (`services/scripts/check_migrations.py`) before
deploying, regardless of where the deploy runs from.
