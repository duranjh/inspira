...
Many registrars' built-in DNS doesn't support CNAME flattening at the apex (you can't `CNAME yourdomain.com → project.pages.dev`). Using Cloudflare's free DNS sidesteps that and gives you faster resolution + free DDoS protection.

1. Create a free Cloudflare account at <https://cloudflare.com>.
2. Dashboard → **Add a site** → `tryinspira.com`. Select the Free plan.
3. Cloudflare scans your existing DNS and shows you the records it found. Just accept.
...
## 11. Create your operator account (2 min)

Sign up through the app's normal signup flow, then set
`INSPIRA_ADMIN_EMAIL` to that account's email so it can read
...
- **Stripe billing** — disabled until `STRIPE_SECRET_KEY` is set (the app
  runs free-tier-for-everyone without it). See the Stripe section above.
- **Transactional email** — set `EMAIL_PROVIDER` + provider API key
  (Resend / Postmark / Loops); no-op otherwise.
- **Error monitoring** — set `SENTRY_DSN_BACKEND` / `VITE_SENTRY_DSN`.
- **Analytics** — set `VITE_PLAUSIBLE_DOMAIN`, or skip entirely.
