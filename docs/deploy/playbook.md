...
- [ ] Rotate any `OPENAI_API_KEY` that has been in your local `.env`.
      The security audit (`docs/deployment/security-audit.md` §C1)
      covers the case where the working-tree `.env` exposed a key.
...
`.github/workflows/deploy.yml` is a scaffold with commented-out jobs for
the production deploy step. When ready, uncomment, wire up the Fly /
